...
import { createMurreMcpServer } from "../src/index.js";
...
test("exposes operator-armed live routing with hard caps and fresh-state gating", async (context) => {
  const directory = await mkdtemp(join(tmpdir(), "murre-live-mcp-"));
  context.after(() => rm(directory, { recursive: true, force: true }));
  const policyPath = join(directory, "policy.json");
  const statePath = join(directory, "state.json");
  const ledgerPath = join(directory, "events.jsonl");
  ...
  const client = new Client({ name: "murre-live-test", version: "1.0.0" });
  ...
  const listed = await client.listTools();
  const liveTool = listed.tools.find((tool) => tool.name === "murre_live_order");
  assert.ok(liveTool);
...
test("reserves live capacity and burns the state snapshot after an ambiguous placement failure", async (context) => {
  const directory = await mkdtemp(join(tmpdir(), "murre-live-failure-"));
  context.after(() => rm(directory, { recursive: true, force: true }));
  const policyPath = join(directory, "policy.json");
  const statePath = join(directory, "state.json");
  const ledgerPath = join(directory, "events.jsonl");
  ...
  const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  const client = new Client({ name: "murre-live-failure-test", version: "1.0.0" });
  context.after(() => clientTransport.close());
  context.after(() => server.close());
  await server.connect(serverTransport);
  await client.connect(clientTransport);
...
test("starts the stdio server in live mode only with the explicit activation contract", async (context) => {
  const directory = await mkdtemp(join(tmpdir(), "murre-live-stdio-"));
  context.after(() => rm(directory, { recursive: true, force: true }));
  const policyPath = join(directory, "policy.json");
  const statePath = join(directory, "state.json");
  const ledgerPath = join(directory, "events.jsonl");
  ...
  const client = new Client({ name: "murre-live-stdio-test", version: "1.0.0" });
  context.after(() => transport.close());
  await client.connect(transport);

  const tools = await client.listTools();
  assert.equal(tools.tools.some((tool) => tool.name === "murre_live_order"), true);
  assert.equal(tools.tools.some((tool) => tool.name === "murre_paper_order"), false);
  assert.equal(tools.tools.some((tool) => tool.name === "murre_rebalance"), false);
  const status = await client.callTool({ name: "murre_status", arguments: {} });
