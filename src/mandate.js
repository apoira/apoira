...
function policyPathFromConfig(config, configPath) {
  if (typeof config.policy !== "string" || config.policy.trim() === "") {
    throw new Error("Murre live config is missing its policy path");
  }
...
export async function updateMandateFiles({
  configPath = ".murre/live-config.json",
  changes = {},
} = {}) {
  ...
  if (policy?.schemaVersion !== "murre.policy.v1") {
    throw new Error("Unsupported Murre policy schema");
  }
  ...
  return {
    schemaVersion: "murre.mandate-update.v1",
    updated: true,
    policy: policyPath,
