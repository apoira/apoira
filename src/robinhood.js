...
export class FileOAuthProvider {
  ...
  constructor({ path, redirectUrl }) {
    ...
    this.data = { schemaVersion: "murre.robinhood-oauth.v1" };
  ...
  async load() {
    try {
      ...
      if (parsed?.schemaVersion !== "murre.robinhood-oauth.v1") {
  ...
  get clientMetadata() {
    return {
      client_name: "Murre",
      client_uri: "https://github.com/murreMCP/murre",
...
export function waitForOAuthCallback({
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((request, response) => {
      ...
      response.end("<!doctype html><title>Murre connected</title><h1>Murre is connected to Robinhood.</h1><p>You can close this tab and return to the terminal.</p>");
      finish(null, code);
    });
