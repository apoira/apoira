...
export async function setupRobinhood({
  ...
  if (portfolio.currency !== "USD") throw new Error("Murre live setup currently requires a USD account");
  ...
  for (const position of positions) {
    ...
    if (quantity < 0 || !["long", "empty"].includes(position.type)) {
      throw new Error(`Murre live setup does not support the ${position.type || "unknown"} ${symbol} position`);
    }
  ...
  const state = {
    ...stateBody,
    snapshotId: `robinhood-${hashWithDomain("murre.robinhood-state.v1", stateBody).slice(0, 24)}`,
  };
  ...
  return {
    schemaVersion: "murre.robinhood-setup.v1",
