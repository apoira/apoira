...
export function validateRobinhoodIntent(input, { accountNumber } = {}) {
  ...
  if (orderType !== "limit") {
    throw new TypeError("Murre live v0.6 permits limit orders only");
  }
  ...
  if (args.time_in_force !== "gfd") {
    throw new TypeError("Murre live v0.6 requires time_in_force gfd");
  }
  if (args.market_hours !== "regular_hours") {
    throw new TypeError("Murre live v0.6 requires regular market hours");
  }
  return { intent, args };
}
...
export async function executeRobinhoodOrder({
  ...
  const reviewHash = hashWithDomain("murre.robinhood-review.v1", rawReview);
  ...
  const placementHash = hashWithDomain("murre.robinhood-placement.v1", rawPlacement);
